...
``` sh
wget -c https://download.fedoraproject.org/pub/fedora/linux/releases/43/Spins/x86_64/iso/Fedora-Xfce-Live-43-1.6.x86_64.iso
```

就會下載一個檔案「Fedora-Xfce-Live-43-1.6.x86_64.iso」。
...
``` sh
sudo dd if=./Fedora-Xfce-Live-43-1.6.x86_64.iso of=/dev/sdc bs=4M status=progress && sync
```

或是執行下面指令，就會寫入「USB隨身碟」。

``` sh
sudo dd if=./Fedora-Xfce-Live-43-1.6.x86_64.iso of=/dev/sdc bs=4M status=progress conv=fsync oflag=direct
```
